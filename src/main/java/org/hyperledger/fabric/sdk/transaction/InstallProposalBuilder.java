...
//            data = Utils.generateTarGz(projectSourceDir, targetPathPrefix, metainf);
            File tarFile = projectSourceDir.listFiles()[0];
            if (tarFile.exists()&&tarFile.isFile()) {
                data = FileUtils.readFileToByteArray(tarFile);
            }else{
                data = null;
                throw new IllegalArgumentException("tar file not exist or tar file is a folder!");
            }
